...
Deno.serve(async (request) => {
  ...
  const { data, error } = await supabase.rpc("admin_create_extra_checkpoint", {
    p_organization_id: device.organization_id,
    p_name: name,
  });

  if (error) {
    if (error.message.includes("PATROL_PLAN_ACTIVE")) return json({ error: "patrol_active" }, 409);
    if (error.message.includes("PLACE_NOT_FOUND")) return json({ error: "place_not_found" }, 409);
    return json({ error: "checkpoint_create_failed", detail: error.message }, 500);
  }
